...
namespace Klei.AI
{
	...
	public class Effect : Modifier
	{
		...
		public static void AddModifierDescriptions(GameObject parent, List<Descriptor> descs, string effect_id, bool increase_indent = false)
		{
			...
			foreach (AttributeModifier selfModifier in effect.SelfModifiers)
			{
				Descriptor item = new Descriptor(Strings.Get("STRINGS.DUPLICANTS.ATTRIBUTES." + selfModifier.AttributeId.ToUpper() + ".NAME") + ": " + selfModifier.GetFormattedString(parent, false), string.Empty, Descriptor.DescriptorType.Effect, false);
				if (increase_indent)
				{
					item.IncreaseIndent();
				}
