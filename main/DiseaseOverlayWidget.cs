	...
	public void Refresh(AmountInstance value_src)
	{
		...
		if (!((Object)gameObject == (Object)null))
		{
			...
			if (sMI != null && sMI.IsSick())
			{
				...
				diseasedImage.color = modifier.overlayColour;
				if (!diseasedImage.enabled)
				{
					diseasedImage.gameObject.GetComponent<ToolTip>().toolTip = modifier.Name;
				}
				diseasedImage.enabled = true;
				progressFill.transform.parent.gameObject.SetActive(false);
				germsImage.transform.parent.gameObject.SetActive(false);
			}
			else
			{
				if (value_src != null)
