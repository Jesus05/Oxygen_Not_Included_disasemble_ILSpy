...
public class LoadScreen : KModalScreen
{
	...
	{
		return header.buildVersion > 298981;
	}
	...
	private void SetSelectedGame(string filename)
	{
		...
		else
		{
			...
			try
			{
				...
				{
					InfoText.text = string.Format(UI.FRONTEND.LOADSCREEN.SAVE_TOO_NEW, filename, header.buildVersion, 298981u);
					loadButton.isInteractable = false;
	...
	private static void DoLoad(string filename)
	{
		...
		string arg = null;
		string arg2 = null;
		if (header.buildVersion > 298981)
		{
			arg = header.buildVersion.ToString();
			arg2 = 298981.ToString();
		}
		else if (gameInfo.saveMajorVersion < 7)
		{
