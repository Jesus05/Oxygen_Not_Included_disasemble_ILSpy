...
public class ConduitFlowVisualizer
{
	...
	public void Render(float z, int render_layer, float lerp_percent, bool trigger_audio = false)
	{
		...
		if (tuning.renderMesh)
		{
			...
			for (int j = 0; j < flowManager.soaInfo.NumEntries; j++)
			{
				int cell = flowManager.soaInfo.GetCell(j);
				Vector2I u = Grid.CellToXY(cell);
				if (!(u < v) && !(u > v2))
				{
					ConduitFlow.Conduit conduit = flowManager.soaInfo.GetConduit(j);
					ConduitFlow.ConduitFlowInfo lastFlowInfo = conduit.GetLastFlowInfo(flowManager);
					ConduitFlow.ConduitContents initialContents = conduit.GetInitialContents(flowManager);
					if (lastFlowInfo.contents.mass > 0f)
					{
						...
						Color32 cellTintColour = GetCellTintColour(cell2);
						Color32 cellTintColour2 = GetCellTintColour(cellFromDirection);
						...
						case ConduitFlow.FlowDirection.Down:
							uvbl = new Vector2I(1, 0);
							uvtl = new Vector2I(0, 0);
							uvbr = new Vector2I(1, 1);
							uvtr = new Vector2I(0, 1);
							break;
						}
						float highlight = 0f;
						if (showContents)
						...
						float num3 = 1f;
						if (showContents || lastFlowInfo.contents.mass < initialContents.mass)
						{
							num3 = CalculateMassScale(lastFlowInfo.contents.mass);
						}
					...
					if (initialContents.mass > lastFlowInfo.contents.mass && initialContents.mass > 0f)
					{
						...
						float highlight2 = 0f;
						Color32 cellTintColour3 = GetCellTintColour(cell3);
						float num4 = CalculateMassScale(mass);
						if (showContents)
						{
							staticBallMesh.AddQuad(pos2, cellTintColour3, tuning.size * num4, 0f, 0f, uvbl2, uvtl2, uvbr2, uvtr2);
						...
						Color32 contentsColor2 = GetContentsColor(element, cellTintColour3);
