...
public class ConduitFlow : IConduitFlow
{
	...
	private struct BuildNetworkTask : IWorkItem<ConduitFlow>
	{
		...
		private struct Graph
		{
			...
			public struct Edge : IEquatable<Edge>
			{
				...
				{
					return (vertices == rhs.vertices && vertices == null) || (vertices.Length == rhs.vertices.Length && vertices.Length == 2 && vertices[0].Equals(rhs.vertices[0]) && vertices[1].Equals(rhs.vertices[1]));
				}
			...
			private ListPool<int, ConduitFlow>.PooledList vertices;

			private ListPool<Edge, ConduitFlow>.PooledList edges;

			private QueuePool<Vertex, ConduitFlow>.PooledQueue traversal;

			private HashSetPool<int, ConduitFlow>.PooledHashSet visited;

			private ListPool<Vertex, ConduitFlow>.PooledList flow_utility_network_roots;

			private ListPool<Vertex, ConduitFlow>.PooledList internal_endpoints;

			private HashSetPool<int, ConduitFlow>.PooledHashSet endpoints;

			public Graph(FlowUtilityNetwork network)
			{
				conduit_flow = null;
				this.network = network;
				vertices = ListPool<int, ConduitFlow>.Allocate();
				edges = ListPool<Edge, ConduitFlow>.Allocate();
				traversal = QueuePool<Vertex, ConduitFlow>.Allocate();
				visited = HashSetPool<int, ConduitFlow>.Allocate();
				flow_utility_network_roots = ListPool<Vertex, ConduitFlow>.Allocate();
				internal_endpoints = ListPool<Vertex, ConduitFlow>.Allocate();
				endpoints = HashSetPool<int, ConduitFlow>.Allocate();
			}
			...
			public void Recycle()
			{
				...
				traversal.Recycle();
				visited.Recycle();
				flow_utility_network_roots.Recycle();
				internal_endpoints.Recycle();
				endpoints.Recycle();
			}

			public void Build(ConduitFlow conduit_flow, FlowUtilityNetwork network, List<FlowUtilityNetwork.IItem> roots)
			{
				this.conduit_flow = conduit_flow;
				flow_utility_network_roots.Clear();
				for (int i = 0; i < roots.Count; i++)
				{
					int cell = roots[i].Cell;
					if (conduit_flow.grid[cell].conduitIdx != -1)
					{
						flow_utility_network_roots.Add(new Vertex
						{
							cell = cell,
							direction = FlowDirections.None
						});
				...
				Build(conduit_flow, network, flow_utility_network_roots);
			}

			public void Build(ConduitFlow conduit_flow, FlowUtilityNetwork network, IEnumerable<Vertex> roots)
			{
				this.conduit_flow = conduit_flow;
				endpoints.Clear();
				foreach (FlowUtilityNetwork.IItem sink in network.sinks)
				{
					endpoints.Add(sink.Cell);
				}
				foreach (FlowUtilityNetwork.IItem source in network.sources)
				{
					endpoints.Add(source.Cell);
				}
				Debug.Assert(traversal.Count == 0);
				foreach (Vertex root in roots)
				{
					traversal.Enqueue(root);
				}
				internal_endpoints.Clear();
				visited.Clear();
				while (traversal.Count != 0)
				{
					Vertex node = traversal.Dequeue();
					vertices.Add(node.cell);
					...
					for (int i = 0; i != num; i++)
					{
						...
						if (new_node.is_valid && new_node.cell != node.cell && !edges.Any((Edge edge) => edge.vertices[0].cell == new_node.cell && edge.vertices[1].cell == node.cell))
						{
							edges.Add(new Edge
							{
							...
							{
								if (IsEndpoint(new_node.cell))
								{
									internal_endpoints.Add(new_node);
								}
								else
								{
									traversal.Enqueue(new_node);
								}
							}
						}
					}
					...
					{
						foreach (Vertex internal_endpoint in internal_endpoints)
						{
							traversal.Enqueue(internal_endpoint);
						}
						internal_endpoints.Clear();
					}
				}
			...
			private bool IsEndpoint(int cell)
			{
				...
				return gridNode.conduitIdx == -1 || endpoints.Contains(cell);
			}
			...
			private Vertex WalkPath(int root_conduit, int conduit, FlowDirections direction)
			{
				if (conduit != -1)
				{
					...
					while (!flag);
					internal_endpoints.Add(new Vertex
					{
						cell = cell,
						direction = ComputeNextFlowDirection(direction)
					});
			...
			public void Merge(Graph inverted_graph)
			{
				...
				for (int i = 0; i != num; i++)
				{
					...
					{
						if (!IsEndpoint(vertex) && !edges.Any((Edge edge) => edge.vertices[0].cell == vertex))
						{
							int num2 = inverted_graph.edges.FindIndex((Edge inverted_edge) => inverted_edge.vertices[1].cell == vertex);
							if (num2 != -1)
							{
		...
		{
			from_sources_graph.Build(outer, network.network, network.network.sources);
			from_sinks_graph.Build(outer, network.network, network.network.sinks);
		...
		{
			for (int i = 0; i < network.network.sources.Count; i++)
			{
				distance_nodes.Enqueue(new DistanceNode
				{
					cell = network.network.sources[i].Cell,
					distance = 0
				});
			}
			...
			for (int j = 0; j < network.network.sinks.Count; j++)
			{
				distance_nodes.Enqueue(new DistanceNode
				{
					cell = network.network.sinks[j].Cell,
					distance = 0
				});
			}
			while (distance_nodes.Count != 0)
	...
	private struct UpdateNetworkTask : IWorkItem<ConduitFlow>
	{
		...
		private DictionaryPool<int, ConduitConsumer, ConduitFlow>.PooledDictionary sinks;

		public UpdateNetworkTask(Network network)
		{
			this.network = network;
			sinks = DictionaryPool<int, ConduitConsumer, ConduitFlow>.Allocate();
			foreach (FlowUtilityNetwork.IItem sink in network.network.sinks)
			{
				sinks.Add(sink.Cell, sink.GameObject.GetComponent<ConduitConsumer>());
			}
		}
	...
	private void UpdateConduit(Conduit conduit, Dictionary<int, ConduitConsumer> sinks)
	{
		...
		else
		{
			...
			if (sinks.TryGetValue(cell, out ConduitConsumer value) && (UnityEngine.Object)value != (UnityEngine.Object)null)
			{
				num2 = Mathf.Max(0f, gridNode.contents.movable_mass - value.capacityKG);
			}
			for (int j = 0; j != 4; j++)
			{
				flowDirections = ComputeNextFlowDirection(flowDirections);
