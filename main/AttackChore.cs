...
public class AttackChore : Chore<AttackChore.StatesInstance>
{
	...
	public AttackChore(IStateMachineTarget target, GameObject enemy)
		: base(Db.Get().ChoreTypes.Attack, target, target.GetComponent<ChoreProvider>(), false, (Action<Chore>)null, (Action<Chore>)null, (Action<Chore>)null, PriorityScreen.PriorityClass.basic, 5, false, true, 0, (Tag[])null)
	{
	...
	public void OnTargetMoved(object data)
	{
		...
		else
		{
			if (smi.GetCurrentState() == smi.sm.attack)
			{
				...
				{
					Debug.Log("has no approachable", null);
				}
			}
			if (multiTool != null)
			{
