...
public class ConsumablesTableScreen : TableScreen
{
	...
	protected override void OnActivate()
	{
		...
		AddLabelColumn("QOLExpectations", on_load_qualityoflife_expectations, get_value_qualityoflife_expectations_label, compare_rows_qualityoflife_expectations, on_tooltip_qualityoflife_expectations, on_tooltip_sort_qualityoflife_expectations, 96, true);
	...
	private void set_value_consumable_info(GameObject widget_go, ResultValues new_value)
	{
		...
		{
			Debug.LogWarning("Row is null", null);
		}
		else
		{
			...
			case TableRow.RowType.Minion:
				if ((UnityEngine.Object)minionIdentity != (UnityEngine.Object)null)
				{
					...
					{
						Debug.LogError("Could not find minion identity / row associated with the widget", null);
					}
					else
					{
						switch (new_value)
	...
	private void on_click_consumable_info(GameObject widget_go)
	{
		...
		case TableRow.RowType.Minion:
			if ((UnityEngine.Object)minionIdentity != (UnityEngine.Object)null)
			{
				...
				{
					consumableUIItem = consumableInfoTableColumn.consumable_info;
					consumableInfoTableColumn.on_set_action(widget_go, (!consumableConsumer.IsPermitted(consumableUIItem.ConsumableId)) ? ResultValues.True : ResultValues.False);
				}
			}
			break;
		}
		...
		else
		{
			...
			case TableRow.RowType.Minion:
				...
				if (foodInfo != null)
				{
					...
					Color color2 = image.color = new Color(0.721568644f, 0.443137258f, 0.5803922f, Mathf.Max((float)foodInfo.Quality - Db.Get().Attributes.FoodExpectation.Lookup(minion).GetTotalValue() + 1f, 0f) * 0.25f);
				}
				break;
			}
			refresh_scrollers();
	...
	private ResultValues get_value_consumable_info(MinionIdentity minion, GameObject widget_go)
	{
		ConsumableConsumer consumableConsumer = null;
		if ((UnityEngine.Object)minion != (UnityEngine.Object)null)
		{
		...
		case TableRow.RowType.Header:
		{
			...
			foreach (KeyValuePair<TableRow, GameObject> item in consumableInfoTableColumn.widgets_by_row)
			{
				...
				{
					switch (consumableInfoTableColumn.get_value_action(item.Key.GetMinionIdentity(), value))
					{
					case ResultValues.False:
						flag2 = false;
						if (!flag)
		...
		case TableRow.RowType.Default:
			...
			break;
		case TableRow.RowType.Minion:
			result = ((!((UnityEngine.Object)minion != (UnityEngine.Object)null)) ? ResultValues.True : (consumableConsumer.IsPermitted(consumable_info.ConsumableId) ? ResultValues.True : ResultValues.False));
			break;
		}
		return result;
	}
