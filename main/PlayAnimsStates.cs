internal class PlayAnimsStates : GameStateMachine<PlayAnimsStates, PlayAnimsStates.Instance, IStateMachineTarget, PlayAnimsStates.Def>
{
	...
	public override void InitializeStates(out BaseState default_state)
	{
		...
		root.ToggleStatusItem("Unused", "Unused", category: Db.Get().StatusItemCategories.Main, icon: string.Empty, icon_type: StatusItem.IconType.Info, notification_type: NotificationType.Neutral, allow_multiples: false, render_overlay: default(HashedString), status_overlays: 63486, resolve_string_callback: (string str, Instance smi) => smi.def.statusItemName, resolve_tooltip_callback: (string str, Instance smi) => smi.def.statusItemTooltip);
		...
		done.BehaviourComplete((Instance smi) => smi.def.tag, false);
	}
}
